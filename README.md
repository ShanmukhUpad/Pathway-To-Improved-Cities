...
- Week 9: (Almost there!) Implement machine learning models to predict outcomes for each dataset
- Week 10: (In Progress) Make new types of visualizations (scatterplots, KDE, etc) and also add a feature to search for datasets via city and provide meaningful insights into each statistic.
- Week 11: (Complete) Implement File Upload System, where any user can upload their own dataset, if it has certain columns, then they can choose what domain the dataset belongs to, and it will be added to the dashboard. Compute Moran I and LISA tests for Spatial Autocorrelation
